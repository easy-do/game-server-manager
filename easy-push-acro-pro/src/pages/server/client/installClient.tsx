...
function InstallClientPage({ id, visible, setVisible, successCallBack }) {
  ...
  function fetchData() {
    if (id !== undefined && visible) {
      setLoading(true);
      setDockerInstall('docker pull registry.cn-hangzhou.aliyuncs.com/gebilaoyu/easy-do-plus:client-1.0.0 && docker run -dit -e CLIENT_ID='+id+' --privileged=true --restart=always --pid=host -v /:/host -v /var/run/docker.sock:/var/run/docker.sock -v $(which docker):/usr/bin/docker --name manager-client registry.cn-hangzhou.aliyuncs.com/gebilaoyu/easy-do-plus:client-1.0.0')
