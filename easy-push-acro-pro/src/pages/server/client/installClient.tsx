...
function InstallClientPage({ id, visible, setVisible, successCallBack }) {
  ...
  function fetchData() {
    if (id !== undefined && visible) {
      setLoading(true);
      setDockerInstall('docker run -dit -e CLIENT_ID='+id+' -v /data/game-manager-client:/data --privileged=true --pid=host -v /:/host -v /var/run/docker.sock:/var/run/docker.sock -v $(which docker):/usr/bin/docker --name manager-client registry.cn-hangzhou.aliyuncs.com/gebilaoyu/game-server-manager:client-1.0.0')
