...
import { managerPage, removeRequest } from '@/api/oauthClient';
...
function SearchTable() {
  ...
  const tableCallback = async (record, type) => {
    //查看
    if (type === 'view') {
      viewInfo(record.clientId);
    }

    //编辑
    if (type === 'update') {
      updateInfo(record.clientId);
    }

    //删除
    if (type === 'remove') {
      removeData(record.clientId);
    }
  };
  ...
  return (
    <Card>
      ...
      <PermissionWrapper
        requiredPermissions={[
          { resource: 'manager:details', actions: ['add'] },
        ]}
      >
