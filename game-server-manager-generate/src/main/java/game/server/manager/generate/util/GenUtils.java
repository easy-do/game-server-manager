...
import game.server.manager.generate.configure.GenerateConfig;
...
public class GenUtils {
    ...
    public static void initTable(GenTable genTable, String operationName) {
        ...
        genTable.setPackageName(GenerateConfig.getPackageName());
        genTable.setModuleName(getModuleName(GenerateConfig.getPackageName()));
        genTable.setBusinessName(getBusinessName(genTable.getTableName()));
        genTable.setFunctionName(replaceText(genTable.getTableComment()));
        genTable.setFunctionAuthor(GenerateConfig.getAuthor());
    }
    ...
    public static void initColumnField(GenTableColumn column, GenTable table) {
        ...
        if (arraysContains(GenConstants.COLUMN_TYPE_STR, dataType) || arraysContains(GenConstants.COLUMN_TYPE_TEXT, dataType)) {
            ...
            List<String> str = CharSequenceUtil.split(CharSequenceUtil.subBetween(column.getColumnType(), "(", ")"), ",");
        ...
        // 类型&性别字段设置下拉框
        else if (CharSequenceUtil.endWithIgnoreCase(columnName, "type")
                || CharSequenceUtil.endWithIgnoreCase(columnName, "sex")) {
            column.setHtmlType(GenConstants.HTML_SELECT);
        }
        // 图片字段设置图片上传控件
        else if (CharSequenceUtil.endWithIgnoreCase(columnName, "image")) {
            column.setHtmlType(GenConstants.HTML_IMAGE_UPLOAD);
        }
        // 文件字段设置文件上传控件
        else if (CharSequenceUtil.endWithIgnoreCase(columnName, "file")) {
            column.setHtmlType(GenConstants.HTML_FILE_UPLOAD);
        }
        // 内容字段设置富文本控件
        else if (CharSequenceUtil.endWithIgnoreCase(columnName, "content")) {
    ...
    public static String getModuleName(String packageName) {
        ...
        return CharSequenceUtil.sub(packageName, lastIndex + 1, nameLength);
    }
    ...
    public static String getBusinessName(String tableName) {
        ...
        return CharSequenceUtil.sub(tableName, lastIndex + 1, nameLength);
    }
    ...
    public static String convertClassName(String tableName) {
        ...
        if (autoRemovePre && CharSequenceUtil.isNotBlank(tablePrefix)) {
            String[] searchList = tablePrefix.split(",");
            tableName = replaceFirst(tableName, searchList);
        }
        return CharSequenceUtil.lowerFirst(CharSequenceUtil.toCamelCase(tableName));
    }
    ...
    public static String getDbType(String columnType) {
        if (CharSequenceUtil.indexOf(columnType, '(') > 0) {
            return CharSequenceUtil.subBefore(columnType, "(", false);
        } else {
            return columnType;
        }
    }
    ...
    public static Integer getColumnLength(String columnType) {
        if (CharSequenceUtil.indexOf(columnType, '(') > 0) {
            String length = CharSequenceUtil.subBetween(columnType, "(", ")");
